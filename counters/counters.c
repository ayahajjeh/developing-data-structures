/* 
 * counters.c - c file for CS50 counters module
 *
...
counters_t* counters_new(void){
    counters_t* newCounterSet = mem_malloc(sizeof(counters_t));
    if (newCounterSet == NULL) return NULL;
    newCounterSet->head = NULL;
    return newCounterSet;
}

counterNode_t* countersFind (counterNode_t* ctrs, const int key){
    if (ctrs == NULL) return NULL;
    if (ctrs->key == key) return ctrs;
    else
        return countersFind(ctrs->nextNode, key);
}
...
int counters_add(counters_t* ctrs, const int key){
    ...
    counterNode_t* node;
    if ((node = countersFind(ctrs->head, key)) == NULL){
        node=mem_malloc(sizeof(counterNode_t));
        if (node == NULL) return 0;
        node->key=key;
        node->count=1;
        node->nextNode=ctrs->head;
        ctrs->head=node;
    }
    else{
        node->count++;
...
void counters_print(counters_t* ctrs, FILE* fp){
    ...
    fprintf(fp, "{");
    for (counterNode_t* node = ctrs->head; node != NULL; node=node->nextNode){
        fprintf(fp, "{%s = %s}, ", node->key, node->count);
    }
    fprintf(fp, "}\n");
}
...
void counters_iterate(counters_t* ctrs, void* arg, 
    ...
    if (ctrs == NULL || itemfunc == NULL || arg == NULL) return;
    for (counterNode_t* node = ctrs->head; node != NULL; node = node->nextNode)
